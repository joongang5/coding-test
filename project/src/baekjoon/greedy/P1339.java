	...
	public static void main(String[] args) {
		...
		for (String word : words) {
			for (int i = 0; i < word.length(); i++) {
				...
				value += Math.pow(10, (word.length() - i)) / 10;
				map.put(ch, value);
			}
		}
		
		
		List<Character> tempList = new ArrayList<Character>(map.keySet());
		tempList.sort((l, r) -> map.get(r) - map.get(l));
		
		long answer = 0;
		for (int i = 0; i < tempList.size(); i++) {
			answer += map.get(tempList.get(i)) * (9 - i);
		}
