...
public class Problum1 {
	...
	public int[] solution(String[] id_list, String[] report, int k) {
        ...
        for (Map.Entry<String, HashSet<String>> entry : reportMap.entrySet()) {
        	HashSet<String> reportSet = entry.getValue();
        	
        	for (String reportId : reportSet) {
            	int reportCount = countMap.getOrDefault(reportId, 0);
            	countMap.put(reportId, ++reportCount);
        	}
        }
        ...
        for (int i = 0; i < id_list.length; i++) {
        	...
        	int mailCount = 0;
        	for (String reportId : reportSet) {
        		int reportCount = countMap.get(reportId);
        		if (reportCount >= k) {
        			mailCount++;
        		}
        	}
